...
OUT_DIR="$NEURON_DIR/generation_output/non_target_coeff_abl_allam"
...
run_one() {
  ...
  local out_file="$out_subdir/allam_${label}_alpha${ALPHA}_gamma$(coeff_label "$MSA_GAMMA")_competitor$(coeff_label "$non_target_gamma").jsonl"

  echo "============================================================"
  echo "ALLaM non-target suppression ablation: ${label}"
