...
OUT_DIR="$NEURON_DIR/generation_output/coeff_ablation_fanar_instruct"
...
run_one() {
  ...
  local label="$5"
  local out_file="$OUT_DIR/fanar_${label}_alpha${alpha}.jsonl"

  echo "============================================================"
  echo "Fanar coefficient ablation: ${label}, alpha=${alpha}"
...
for alpha in 1 2 3 4 5; do
  run_one "$alpha" "$EVAL_DIR/egy.jsonl" CAI RAB "egy_to_cairo"
  run_one "$alpha" "$EVAL_DIR/mar.jsonl" RAB CAI "mar_to_rabat"
done
