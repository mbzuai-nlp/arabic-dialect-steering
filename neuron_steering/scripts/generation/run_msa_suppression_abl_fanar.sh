...
OUT_DIR="$NEURON_DIR/generation_output/MSA_coeff_abl_fanar"
...
run_one() {
  ...
  local out_file="$out_subdir/fanar_${label}_alpha${ALPHA}_gamma$(gamma_label "$gamma").jsonl"

  echo "============================================================"
  echo "Fanar MSA suppression ablation: ${label}, alpha=${ALPHA}, gamma=${gamma}"
