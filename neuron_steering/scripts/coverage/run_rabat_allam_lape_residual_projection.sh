...
"$PYTHON_BIN" "$NEURON_DIR/measure_lape_residual_projection.py" \
  ...
  --vector_path "$REPO_DIR/arabic_steering_vector/dialect_vectors/ALLaM-7B-Instruct-preview/Rabat_response_avg_diff.pt" \
  --target_dialect RAB \
  --out_dir "$NEURON_DIR/coverage_output/allam_rabat_residual_projection" \
  --dtype auto \
  --device_map auto
