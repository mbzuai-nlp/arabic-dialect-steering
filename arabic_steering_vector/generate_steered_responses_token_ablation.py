"""
Generate baseline and steered responses ablating over the number of steered tokens.

Like generate_steered_responses.py but adds a sweep over --n-steer-tokens:
for each value N the steering vector is applied only to the first N decoded
tokens; the remainder of the response is generated unsteered.  Use N=0 to
mean "steer all tokens" (no budget cap).

Output file naming: results_layers/{dialect}_layer{L}_steer{N}tok.jsonl
  where N is the token budget (or "all" when 0 / uncapped).

Usage:
    python generate_steered_responses_token_ablation.py \\
...
def generate_steered(model, tokenizer, prompt_text, vector, layer_idx, coef,
                     n_steer_tokens, max_new_tokens, temperature):
    inputs = tokenizer(prompt_text, return_tensors="pt").to(model.device)
    # n_steer_tokens=None means steer all tokens (no budget cap)
    token_budget = None if n_steer_tokens == 0 else n_steer_tokens
    with ActivationSteerer(
        model, vector,
        coeff=coef,
        layer_idx=layer_idx,
        positions="response",
        n_steer_tokens=token_budget,
    ):
        with torch.no_grad():
            output_ids = model.generate(
                **inputs,
...
def main():
    parser = argparse.ArgumentParser(
        description="Steer responses ablating over number of steered tokens",
    ...
    parser.add_argument(
        ...
        help="Layer indices to run (e.g. --layers 8 12 16). Ignored if --all-layers is set.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n-steer-tokens", nargs="+", type=int, default=[0, 5, 10, 20, 50],
        help=(
            "Token budgets to ablate over. "
            "0 = steer all tokens (no cap). "
    ...
    parser.add_argument("--name", type=str, default=None,
                        help="Override output filename prefix (default: dialect field from eval file)")
    ...
    dialect = args.name if args.name else samples[0].get("dialect", eval_file.stem)
    print(f"\nDialect:      {dialect}")
    print(f"Samples:      {len(samples)}")
    print(f"Token budgets: {args.n_steer_tokens}  (0 = all tokens steered)")
    ...
    baselines = []
    for sample, prompt_text in tqdm(zip(samples, formatted_prompts), total=len(samples)):
        response = generate(model, tokenizer, prompt_text, args.max_tokens, args.temperature)
        baselines.append(response)

    # ─── Sweep: layer × n_steer_tokens ────────────────────────────────────────
    for layer_idx in layers:
        ...
        for n_tok in args.n_steer_tokens:
            label = "all" if n_tok == 0 else str(n_tok)
            print(f"\n{'='*60}")
            print(f"Layer {layer_idx}  |  token budget: {label}")
            print(f"{'='*60}")

            out_path = Path(args.output_dir) / f"{dialect}_layer{layer_idx}_steer{label}tok.jsonl"
            with open(out_path, "w", encoding="utf-8") as out_f:
                for sample, prompt_text, baseline in tqdm(
                    ...
                    record = {
                        ...
                        "source": dialect,
                        "layer": layer_idx,
                        "n_steer_tokens": None if n_tok == 0 else n_tok,
