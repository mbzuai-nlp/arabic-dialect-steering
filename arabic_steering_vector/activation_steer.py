...
class ActivationSteerer:
    ...
    def _hook_fn(self, module, ins, out):
        ...
        def _add(t):
            # If n_steer_tokens is set, gate decoding steps by the budget.
            # Prefill (seq_len > 1) is unaffected and follows positions logic below.
            if self.n_steer_tokens is not None and t.shape[1] == 1:
                if self._tokens_steered >= self.n_steer_tokens:
                    return t  # budget exhausted — skip this decoding step
                self._tokens_steered += 1
                # fall through to apply the normal positions steer
